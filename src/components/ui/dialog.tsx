...
export function DialogHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn("space-y-1 border-b border-hairline px-5 py-4 pr-12", className)}
      {...props}
    />
  );
}
...
export function DialogFooter({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "flex flex-col-reverse gap-2 border-t border-hairline bg-navy-50/60 px-5 py-3.5 sm:flex-row sm:justify-end",
        className,
      )}
      {...props}
    />
